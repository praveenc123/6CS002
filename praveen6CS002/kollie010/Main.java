...
/**
 * @author Kevan Buckley, maintained by Elvin Kollie
 * @version 2.0, 2014
 */
...
public class Main {
  ...
  public List<Domino> ListofDomino;
  ...
  private void generateDominoes() {
	ListofDomino = new LinkedList<Domino>();
    int count = 0, x = 0, y = 0;
    for (int l = 0; l <= 6; l++) {
      for (int h = l; h <= 6; h++) {
  ...
  private void shuffleDominoesOrder() {
    ...
    while (ListofDomino.size() > 0) {
      int n = (int) (Math.random() * ListofDomino.size());
      shuffled.add(ListofDomino.get(n));
      ListofDomino.remove(n);
    }

    ListofDomino = shuffled;
  }
...
//    System.out.println("Serial number " + Special.getStamp());
    ...
    while (_$_ != ZERO) {
      ...
      switch (_$_) {
      case 5:
        int index = (int) (Math.random() * (Sandown.toad.length / 3));
        String what = Sandown.toad[index * 3];
        String who = Sandown.toad[1 + index * 3];
        System.out.printf("%s said \"%s\"", who, what);
        System.out.println();
        System.out.println();
        break;      
      case 0: {
        if (ListofDomino == null) {
      ...
      case 3: {
        ...
        JFrame f = new JFrame("Rules by Elvin Kollie");
  ...
  public static int gecko(int _z) {
    if (_z == (32 & 16)) {
      return -7;
    } else {
      if (_z < 0) {
